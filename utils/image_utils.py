...
def data_augmentation(image, mode):
    ...
    if isinstance(image, torch.Tensor):
        image = image.cpu().numpy()  # 转换为 NumPy 数组
    ...
    if mode == 0:
        ...
        out = np.rot90(image, k=1, axes=(0, 1))  # 旋转90度，轴为空间维度
    elif mode == 3:
        # rotate 90 degrees and flip up and down
        out = np.flipud(np.rot90(image, k=1, axes=(0, 1)))
    elif mode == 4:
        # rotate 180 degrees
        out = np.rot90(image, k=2, axes=(0, 1))
    elif mode == 5:
        # rotate 180 degrees and flip
        out = np.flipud(np.rot90(image, k=2, axes=(0, 1)))
    elif mode == 6:
        # rotate 270 degrees
        out = np.rot90(image, k=3, axes=(0, 1))
    elif mode == 7:
        # rotate 270 degrees and flip
        out = np.flipud(np.rot90(image, k=3, axes=(0, 1)))
    else:
        raise Exception('Invalid choice of image transformation')

    # 确保输出形状与输入相同
    assert out.shape == image.shape, f"Output shape {out.shape} does not match input shape {image.shape}"
